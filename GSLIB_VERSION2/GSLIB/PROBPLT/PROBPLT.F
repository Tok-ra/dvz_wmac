...
c
c-----------------------------------------------------------------------
      parameter (MAXDAT= 500000, MV=20, NPVAL = 21, ZERO=0.0, 
     +           BIGNUM=1.0e21, EPSLON=1.0e-21, VERSION=2.000)

      real       ar1(MAXDAT),ar2(MAXDAT),var(MV),xx(MV),yy(MV),
     +           pval(NPVAL)
      real*8     pdp
      character  datafl*40,outfl*40,varlab*24,title*40,str*80
...
c
      ...
      if(ilog.eq.1) then
            pmin   = max(ar1(1), 0.0001)
            pmax   = max(ar1(nd),0.0001)
